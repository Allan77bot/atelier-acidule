...
- 🆕 **`src/pages/catalogue.astro` (2026-07-31)** — la galerie complète, alimentée par `src/data/catalogue.ts` (223 photos du Drive « Photo sac », converties en WebP et rangées par famille). N'affiche que les **179 photos produit** : `divers` (réserve), `homepage` et `racine` (logos) sont exclus. Barre de familles sticky, grille en **vignettes 480 px** (`*-min.webp`) et pleine résolution réservée à la visionneuse `<dialog>` (Échap et piège à focus natifs, flèches ←/→, retour du focus sur la vignette d'origine). Layout `PageVoeu`.
- 🆕 **`src/data/catalogue.ts`** — **fichier généré**, ne pas éditer à la main : slug de famille, libellé (repris des noms de dossiers de la cliente), chemins `src`/`thumb` et dimensions. Régénéré par les scripts de session (`convert.py` + `thumbs.py` + `gen_catalogue.py`).
- 🆕 **`public/videos/`** — boucle d'ambiance de `/atelier` (`atelier-citrons.mp4` 179 Ko + `.webm` 76 Ko, poster `images/atelier-citrons-poster.webp`). Muette, `playsinline`, **arrêtée sous `prefers-reduced-motion`** (le poster reste seul).
- `src/pages/` — une page par route (`index`, `mes-creations`, `catalogue`, `editions-speciales`, `atelier`, `commander`). **3 layouts actifs** selon la page (voir plus bas). `index.astro` = la **vitrine longue** de l'accueil (nouvelle DA maquettes) : header + footer **inline** dans la page (pas les composants `Nav`/`Footer`) + CSS + **3 `<script>`** (burger, carrousel d'avis, reveal au scroll). `mes-creations.astro` = l'expérience split-screen GSAP (markup + CSS + script GSAP en un seul fichier) — c'est l'**ancien** accueil.
